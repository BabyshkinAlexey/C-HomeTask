...
int[,] RemoveRowColumsMatrix(int[,] matrix, int indexRow, int indexColumn)
{
    int rows = matrix.GetLength(0);
    int columns = matrix.GetLength(1);
    ...
    for (int i = 0; i < rows-1; i++)
    {
        ...
        for (int j = 0; j < columns-1; j++)
        {
            ...
            n++;
            // if (i != indexRow && j != indexColumn) matrix2[m,n] = matrix[i,j];
        }
        m++;
    }
    return matrix2;
...
int[,] matrix3 = RemoveRowColumsMatrix(matrix1,rowIndex1,columnIndex1);
PrintMatrix(matrix3);